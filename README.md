...
To support additional file formats, update the `DocumentProcessor` class in `ra_workflow.py`:
...
Modify the prompt in `ScaffoldPromptAgent` to adjust question style, difficulty, or focus areas.

### Extending the Framework

Add new dimensions or modify existing ones by updating:
- `getDimensionName()` function in the React component
- Question generation prompts in the workflow
...
- **Reading Apprenticeship** framework by WestEd
...
4. Verify your Google API key is configured correctly
